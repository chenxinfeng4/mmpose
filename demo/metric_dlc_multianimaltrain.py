...
basedir = '/home/liying_lab/chenxinfeng/deeplabcut-project/bwrat_14ratkptMulti-cxf-2022-03-01/labeled-data/2021-11-02-bwrat_side6-kp_trainval/val/'
...
det_file = osp.join(
    basedir, 'valDLC_resnet101_bwrat_14ratkptMultiFeb26shuffle1_200000_el.h5')
...
for (df_det_xnow, df_det_ynow, df_det_pnow, df_gt_xnow, df_gt_ynow) \
    ...
    d1 = np.sqrt(
        np.square(df_det_xnow - df_gt_xnow) +
        np.square(df_det_ynow - df_gt_ynow))
    d1[np.isnan(d1)] = 5
    ...
    d2 = np.sqrt(
        np.square(df_det_xnow2 - df_gt_xnow) +
        np.square(df_det_ynow2 - df_gt_ynow))
    d2[np.isnan(d2)] = 5
